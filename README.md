...
| `build_committee.py` + `committee_template.html` | Builds `site/ledger_committee.html`, the Ledger Committee: seven AI seats (quality, value, risk, skeptic, macro, two chairs) deliberate live on the page over the Ledger's candidates to propose a portfolio for the brief you fill in, with a risk profile (Calm, Moderate, High, Extreme) whose own measured record sits behind each button. Extreme is a moonshot meeting: small and micro caps only, including not-yet-profitable companies from `data/universes/speculative.tsv`, with the seats switched to growth jobs (revenue acceleration, cash runway, dilution) and the pool's own base rates (how often a small stock doubled, tripled or halved in a year since 2020). Needs to be opened inside the Claude app to run; elsewhere it shows an example session. |
